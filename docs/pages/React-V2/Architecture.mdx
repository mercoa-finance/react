...
1. **Parent Component** (e.g., `Payables`)

   - Accepts **query options, configuration, display settings, event handlers, and UI overrides** as props.
   - Passes these to a **custom hook (`usePayables`)** for processing.

2. **Internal State Hook (`usePayables`)**

   - Manages **data fetching, state, and computed values**.
   - Returns processed data and functions.

3. **Internal Context Provider (`PayablesProvider`)**

   - Stores and provides data returned from the hook.
   - Exposes the data and actions to child components using `usePayablesContext`.

4. **Child Components (`PayableForm`, `PayableDocument`, etc.)**
   - Consume `usePayablesContext` to access data without needing direct props.
...
### Custom Hook (`usePayables`)

Handles data fetching, state management, and computed values.

```tsx
export const usePayables = (queryOptions?: PayablesQueryOptions) => {
  // Internal logic for fetching data, handling state, and providing computed values
}
```

### Context Provider (`PayablesProvider`)

Exposes data and utility functions to child components.

```tsx
const PayablesContext = createContext<PayablesContextValue | null>(null)

export const PayablesProvider: FC<PayablesProviderProps> = ({ queryOptions, children }) => {
  const payables = usePayables(queryOptions)
...
export const usePayablesContext = () => {
  ...
  if (!context) {
    throw new Error('usePayablesContext must be used within PayablesProvider')
  }
  return context
}
```
...
export const PayableForm = () => {
  const { payables, loading } = usePayablesContext()
...
```
 ┌──────────────┐    Calls Hook      ┌─────────────────┐
 │  Payables    │  ───────────────►  │  usePayables    │
 └──────────────┘                    └─────────────────┘
           │                                 │
      Provides Context                   Returns
           ▼                                 ▼
 ┌──────────────────┐    Access via   ┌───────────────────┐
 │  Child Component │  ◄───────────  │ usePayablesContext │
 └──────────────────┘                 └───────────────────┘
```
