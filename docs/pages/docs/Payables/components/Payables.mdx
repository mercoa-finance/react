...
> **What's New:**  
> The `<Payables>` component now supports custom grouping and labeling of status tabs via the new `customStatuses` prop in `displayOptions`. This allows you to define your own tab labels and map multiple invoice statuses to each tab for a more tailored dashboard experience.
...
| Name       | Type      | Required | Description                         |
| ---------- | --------- | -------- | ----------------------------------- |
| `readOnly` | `boolean` |          | Indicates if the part is read-only. |
