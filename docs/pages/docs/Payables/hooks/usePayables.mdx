...
The `usePayables` hook works inside the children of the `<Payables>` component. It provides access to payables data, filtering capabilities, pagination controls, row selection, action handlers, and advanced customization of status tabs.
