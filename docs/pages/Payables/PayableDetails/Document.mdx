...
| onDocumentUpload      | `(document: { fileReaderObj: string; mimeType: string }) => void`                    |          | Callback when the document is uploaded                                                                          |
...
<ComponentContainer>
 <OcrProgressBar ocrProcessing={true} />
  ```jsx
  <OcrProgressBar ocrProcessing={true} />
  ```
</ComponentContainer>
...
| sourceEmails          | `Mercoa.EmailLog[]`                                                                 |          | Incomming emails related to the invoice |
