import { Canvas, Meta, Source, Markdown } from '@storybook/blocks'

import * as PayablesTable from './PayablesTable.stories'

<Meta of={PayablesTable} />

# Invoice Table

The invoice table renders a list of invoices that can be selected or actioned on

## Default Table Layout

<Canvas of={PayablesTable.Default} />

## Custom Columns

<Canvas of={PayablesTable.Column} />

## Custom Design

<Canvas of={PayablesTable.Custom} />

`````jsx
  <PayablesTable {...params}>
  ({ invoices, hasNext, getNext, hasPrevious, getPrevious, setResultsPerPage }) => {
      return (
        <div style={{ maxWidth: '800px', margin: 'auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '5px' }}>
            {invoices.map((invoice) => (
              <div key={invoice.id} style={{ padding: '5px', border: '1px solid #eee' }}>
                <p style={{ fontWeight: 'bold' }}>{invoice.vendor?.name ?? 'N/A'}</p>
                <p style={{ textDecoration: 'underline' }}>{invoice.metadata.projectId}</p>
                <p>Amount: ${invoice.amount}</p>
                <p>Due: {invoice.dueDate?.toDateString()}</p>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '5px' }}>
            <button
              style={{ padding: '5px', border: '1px solid #eee', color: hasPrevious ? '#000' : '#ccc' }}
              disabled={!hasPrevious}
              onClick={getPrevious}
            >
              Previous
            </button>
            <button
              style={{ padding: '5px', border: '1px solid #eee', color: hasNext ? '#000' : '#ccc' }}
              disabled={!hasNext}
              onClick={getNext}
            >
              Next
            </button>
          </div>
        </div>
      )
    }
  </InvoiceTable>
```import { Canvas, Markdown, Meta, Source } from '@storybook/blocks'

````import { Canvas, Markdown, Meta, Source } from '@storybook/blocks'

`````
