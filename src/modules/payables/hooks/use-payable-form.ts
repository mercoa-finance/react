...
import { OcrJobStatus } from '@mercoa/javascript/api'
...
export const usePayableDetails = ({
  ...
  const { data: ocrJob } = useGetOcrJob(invoiceData?.ocrJobId ?? activeOcrJobId)
  ...
  const usePaymentMethodOnce = useRef(true)
  ...
  // @ts-ignore - Ignoring typescript issue for this block
  const [
    amount,
    description,
    deductionDate,
  ...
  }, [mercoaSession, ocrResponse, setOcrResponse, setValue])
  ...
  useEffect(() => {
    if (!!ocrJob) {
      if (ocrJob.status === OcrJobStatus.Success) {
        setOcrProcessing(false)
        setActiveOcrJobId(undefined)
        setOcrResponse(ocrJob.data)
        return
      }
      if (ocrJob.status === OcrJobStatus.Failed) {
  ...
  useEffect(() => {
    ...
    if (!usePaymentMethodOnce.current) return
    usePaymentMethodOnce.current = false
  ...
  useEffect(() => {
    approvers.forEach((approverSlot: any, index: any) => {
  ...
  const handleCreateOrUpdatePayable = async (formData: PayableFormData, action: PayableAction) => {
    ...
    requestPayload = onInvoicePreSubmit ? await onInvoicePreSubmit?.(requestPayload) : requestPayload
          ...
          },
          {
            ...
            onError: async (e) => {
              ...
              if (
                ...
                ![PayableAction.APPROVE, PayableAction.REJECT].includes(action as any)
              ) {
  ...
  const filteredComments = comments?.filter((comment: any) => comment.text || comment.associatedApprovalAction) ?? []
  ...
  const initialCreationComment = {
    ...
    associatedApprovalAction: {
      ...
      userId: (watch('creatorUser') as any)?.id ?? '',
    },
  }
  ...
  const handleUpdateTotalAmount = () => {
    ...
    lineItems?.forEach((lineItem: any, index: any) => {
  ...
  const updateItem = (index: number, item: Mercoa.InvoiceLineItemUpdateRequest, id?: string) => {
    if (id) {
      const index = lineItems.findIndex((ele: any) => ele.id === id)
      if (index !== -1) {
  ...
  useEffect(() => {
    ...
    const total = lineItems.reduce((sum: any, item: any) => sum + (item.amount ?? 0), 0)
    if (total !== amount) {
  ...
  const overviewContext: PayableOverviewContext = {
    ...
    setPrintDescriptionOnCheckRemittance: (printDescriptionOnCheckRemittance: boolean) =>
      setValue('paymentDestinationOptions.printDescription' as any, printDescriptionOnCheckRemittance),
  }
  ...
  const commentsContext: PayableCommentsContext = {
    ...
    commentText: watch('commentText') as any,
  ...
  const taxAndShippingContext: PayableTaxAndShippingContext = {
    taxAmount: watch('taxAmount') as any,
    setTaxAmount: (taxAmount: number) => setValue('taxAmount', taxAmount),
    shippingAmount: watch('shippingAmount') as any,
  ...
  const recurringScheduleContext: RecurringScheduleContext = {
    ...
    setType: (type: 'weekly' | 'monthly' | 'yearly' | 'daily' | 'oneTime') => setValue('paymentSchedule.type' as any, type),
    setRepeatEvery: (repeatEvery: number) => setValue('paymentSchedule.repeatEvery' as any, repeatEvery),
    setRepeatOn: (repeatOn: Array<Mercoa.DayOfWeek>) => setValue('paymentSchedule.repeatOn' as any, repeatOn),
    setRepeatOnDay: (repeatOnDay: number) => setValue('paymentSchedule.repeatOnDay' as any, repeatOnDay),
    setRepeatOnMonth: (repeatOnMonth: number) => setValue('paymentSchedule.repeatOnMonth' as any, repeatOnMonth),
    setEnds: (ends: Date | undefined) => setValue('paymentSchedule.ends' as any, ends),
  }
  ...
  const approversContext: PayableApproversContext = {
    ...
    setApproverBySlot: (approvalSlotId: string, assignedUserId: string) => {
      const index = approvers.findIndex((approver: any) => approver.approvalSlotId === approvalSlotId)
      if (index !== -1) {
    ...
    getApprovalSlotOptions: (approvalSlotId: string) => {
      const slot = approvers.find((approver: any) => approver.approvalSlotId === approvalSlotId)
      if (!slot) return []
      return [
        { disabled: false, value: { id: 'ANY', name: 'Any Approver', email: '' } },
        ...filterApproverOptions({
          approverSlotIndex: approvers.findIndex((a: any) => a.approvalSlotId === approvalSlotId),
    ...
    selectedApproverBySlot: (approvalSlotId: string) => {
      return (
        [...mercoaSession.users, { id: 'ANY', name: 'Any Approver', email: '' }].find((user: any) => {
          const approverSlot = approvers.find((e: any) => e?.approvalSlotId === approvalSlotId)
