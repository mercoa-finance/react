...
export const useReceivableDetailsInternal = ({ queryParams, toast, handlers, config }: UseReceivableDetailsProps) => {
  ...
  const handleFormSubmit = async (data: any) => {
    ...
    if (receivableData && [ReceivableFormAction.SendEmail, ReceivableFormAction.SaveDraft].includes(data.formAction)) {
      const response = await mercoaSession.client?.invoice.update(
        receivableData.id,
        data.formAction === ReceivableFormAction.SendEmail ? { ...newInvoice, status: 'APPROVED' } : newInvoice,
      )
      ...
      if (response?.id) {
        ...
        if (data.formAction === ReceivableFormAction.SaveDraft) {
          setValue('formAction', '')
        }
      } else {
  ...
  const handleActionClick = async (action: ReceivableFormAction) => {
    ...
    try {
      switch (action) {
        ...
        case ReceivableFormAction.PaymentLink:
        ...
        case ReceivableFormAction.SendEmail:
        case ReceivableFormAction.ResendEmail:
          if (!receivableData?.id || !receivableData.payer) {
            toast.error('There is no payer associated with this invoice. Please select a payer and save draft.')
            return
          }
        ...
        case ReceivableFormAction.MarkAsPaid:
          if (!receivableData?.id) return
          try {
          ...
          break
        case ReceivableFormAction.SchedulePayment:
          if (!receivableData?.id) return
          try {
        ...
        case ReceivableFormAction.RestoreAsDraft:
          if (!receivableData?.id) return
