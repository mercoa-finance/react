...
export function ReceivableFormErrorsV1() {
  const {
    formState: { errors },
    clearErrors,
  } = useFormContext()
  ...
  const keyToField = {
    ...
    'lineItems.0.name': 'Line Item Name:',
    'lineItems.0.quantity': 'Line Item Quantity:',
    'lineItems.0.unitPrice': 'Line Item Unit Price:',
    'lineItems.0.amount': 'Line Item Amount:',
  } as Record<string, string>

  const errorMessages = {
    lineItems: 'Please make sure all line items have a name and valid amounts',
    payerId: 'Please select a customer',
    amount: 'Please add line items to the invoice',
  } as Record<string, string>
